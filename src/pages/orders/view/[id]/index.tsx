...
function OrderViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrderInterface>(
    ...
    () =>
      getOrderById(id, {
        relations: ['user_order_customer_idTouser', 'user_order_vendor_idTouser', 'order_item'],
      }),
  );
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Order Vendor Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_order_vendor_idTouser?.id}`}>
                      {data?.user_order_vendor_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
